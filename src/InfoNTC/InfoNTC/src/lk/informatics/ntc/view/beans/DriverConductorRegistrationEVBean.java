...
@ManagedBean(name = "editViewDriverConductorRegistrationBean")
@ViewScoped
public class DriverConductorRegistrationEVBean {
	...
	public void onTrainingTypeChange() {
		...
		if (strSelectedType != "") {
			disableNic = false;
			idNoList = driverConductorTrainingService.getPendingIDListByTrainingType(strSelectedType);
			idNoListForView = driverConductorTrainingService.getPendingIDListForViewByTrainingType(strSelectedType);
			driverConList = driverConductorTrainingService.getPendingDCIDListByTrainingType(strSelectedType);
			driverConListForView = driverConductorTrainingService
					.getPendingDCIDListByTrainingTypeForView(strSelectedType);
			receiptList = driverConductorTrainingService.getReceiptNoForDriverConductor(strSelectedType);
		}
	}

	public void onTrainingTypeChangeView() {
		duplicateTraining = false;
		photoHave = false;
		if (strSelectedType != "") {
			disableNic = false;
			...
			driverConList = driverConductorTrainingService.getPendingDCIDListByTrainingandID(strSelectedType,
					strSelectedNic);
			driverConListForView = driverConductorTrainingService
					.getPendingDCIDListByTrainingandIDForView(strSelectedType, strSelectedNic);
			receiptList = driverConductorTrainingService.getReceiptNoForDriverConductor(strSelectedType);
...
// for View Page 
	/***
	 * User should be able to search by Id Number Request By Damith /Pramitha on
	 * 17/11/2022
	 ***/
	public void onIDNoChangeNew() {

		if (strSelectedNic != "") {
			disableType = false;
			disableDCId = false;
			trainingTypeList = driverConductorTrainingService.getTrainingTypeByNic(strSelectedNic);
	...
	public void update() throws ParseException {
		...
		if (!receiptGenerated) {
			...
			if (validateForm()) {
				...
				boolean isUpdated = driverConductorTrainingService.updateDriverConductorRegistrationEV(registrationDto,
						strSelectedType, loginUser);

				if (isUpdated) {

					driverConductorTrainingService.beanLinkMethod(registrationDto, loginUser,
							"Edit Driver/Conductor Registration Data", "Edit Driver/Conductor Registration");
	...
	public void goToDocumentsManagementBtn() {
		...
		try {
			...
			driverConductorTrainingService.beanLinkMethod(registrationDto, loginUser, "Document Submit",
					"Edit Driver/Conductor Registration");
		} catch (Exception e) {
			e.printStackTrace();
		}
