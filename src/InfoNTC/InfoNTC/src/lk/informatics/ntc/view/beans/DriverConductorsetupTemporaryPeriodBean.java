...
@ManagedBean(name = "driverConductorSetupTempPeriodBean")
@ViewScoped

public class DriverConductorsetupTemporaryPeriodBean {
	...
	private boolean noDisplayCheckBox ;
	private List<DriverConductorRegistrationDTO> selectDTOList;
	@PostConstruct
	public void init() {
		...
		idNoList = driverConductorTrainingService.getTempANICListN();
		...
		noDisplayCheckBox =true;
	}
	public boolean isNoDisplayCheckBox() {
		return noDisplayCheckBox;
	}
	public void setNoDisplayCheckBox(boolean noDisplayCheckBox) {
		this.noDisplayCheckBox = noDisplayCheckBox;
	}
	...
	{		
		...
		{
			disableNic= false;
			idNoList = driverConductorTrainingService.getTempANICListByTrainingTypeN(strSelectedType);
	...
	boolean dataInsert =false;
		...
		if(strSelectedNic!=null && !strSelectedNic.trim().isEmpty() && strSelectedType!=null && !strSelectedType.trim().isEmpty()) {
			showSearchData = new ArrayList<>();
			showSearchData=driverConductorTrainingService.getBySearcgPaymentCompletedDC(strSelectedType,strSelectedNic,strSelectedID,strName);
			...
			/***modification by tharushi.e 20/01/2020** for render save button in search***/
			dataInsert=driverConductorTrainingService.getDataInDriverConductortemp(strSelectedNic,strSelectedID);
			if(dataInsert) {
			...
			/***modification by tharushi.e 20/01/2020** for render save button in search end ***/
	...
	public void selectRow() {
		if(selectDTO.getAppNo() !=null) {
		noDisplayCheckBox = false;
		}
	}
	public void saveTempPeriodData() {

		if (!selectDTOList.isEmpty()) {
			int returnVal =0;
			returnVal=	dataCheckingMethod(selectDTOList);
			...
			for(DriverConductorRegistrationDTO data : selectDTOList) {
				saveData.setAppNo(data.getAppNo());
				saveData.setNic(data.getNic());
				saveData.setDriverConductorId(data.getDriverConductorId());
				
				driverConductorTrainingService.beanLinkMethod(saveData, sessionBackingBean.getLoginUser(), "Save Temp Period Data", "Setup Temporary Period - Driver/Conductor");
			}
	...
	public void updateDates() {
		...
		if (driverConductorTrainingService.updateDCTempPeriod(selectDTO, sessionBackingBean.getLoginUser())) {
			...
			driverConductorTrainingService.beanLinkMethod(selectDTO, sessionBackingBean.getLoginUser(), "Update Temp Period Data", "Setup Temporary Period - Driver/Conductor");
			setSuccessMessage("Successfully Updated.");
			RequestContext.getCurrentInstance().update("successMSG");
			RequestContext.getCurrentInstance().execute("PF('successMessage').show()");
			RequestContext.getCurrentInstance().update("frmsuccess");
	...
	public List<DriverConductorRegistrationDTO> getSelectDTOList() {
		return selectDTOList;
	}
	public void setSelectDTOList(List<DriverConductorRegistrationDTO> selectDTOList) {
