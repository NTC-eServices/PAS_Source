...
@ManagedBean(name = "permitRenewalsBackingBean")
@ViewScoped
public class PermitRenewalsBackingBean {
	...
	private PermitRenewalsDTO ExpiredDateDTO;
	...
	public List<PermitRenewalsDTO> filterApplalicationNoList = new ArrayList<PermitRenewalsDTO>(0);
	public List<PermitRenewalsDTO> filterPermitNoList = new ArrayList<PermitRenewalsDTO>(0);
	private List<PermitRenewalsDTO> getPreAppNoForLatestInActiveStatusList = new ArrayList<PermitRenewalsDTO>(0);
	...
	private PermitRenewalsDTO applicationHistoryTranDTO;
	...
	public void init() {
		...
		permitNoList = permitRenewalsService.getAllPermitNoList();
		applicationNoList = permitRenewalsService.getAllApplicationNoList();
	...
	public PermitRenewalsBackingBean() {
		...
		applicationHistoryTranDTO = new PermitRenewalsDTO();
	}

	public void filteredValuesList() {
		permitNoList = permitRenewalsService.getAllPermitNoList();
		applicationNoList = permitRenewalsService.getAllApplicationNoList();
		filterApplalicationNoList = new ArrayList<PermitRenewalsDTO>(0);

		for (int i = 0; i < applicationNoList.size(); i++) {
			String currentApplicationNo = applicationNoList.get(i).getApplicationNo();
			String currentQueueNo = applicationNoList.get(i).getQueueNo();

			boolean valueOne = true;
			valueOne = migratedService.findCancelledQueueNumber(currentQueueNo);

			if (valueOne == true) {
				permitRenewalsDTO.setApplicationNo(currentApplicationNo);
				filterApplalicationNoList.add(permitRenewalsDTO);
				break;
			}
		}
		filterPermitNoList = new ArrayList<PermitRenewalsDTO>(0);

		for (int i = 0; i < permitNoList.size(); i++) {
			String currentPermitNo = permitNoList.get(i).getPermitNo();
			String currentQueueNo = permitNoList.get(i).getQueueNo();

			boolean valueOne = true;
			valueOne = migratedService.findCancelledQueueNumber(currentQueueNo);

			if (valueOne == true) {
				permitRenewalsDTO.setPermitNo(currentPermitNo);
				filterPermitNoList.add(permitRenewalsDTO);
				break;
			}
		}
	}
	...
	public void onPermitNoChange() {
		detailsForSearchedPermitNo = permitRenewalsService.getAllDetailsForSearchedPermitNo(selectedPermitNo);
		permitRenewalsDTO.setApplicationNo(detailsForSearchedPermitNo.getApplicationNo());
		setSelectedApplicationNo(permitRenewalsDTO.getApplicationNo());
		permitRenewalsDTO.setQueueNo(detailsForSearchedPermitNo.getQueueNo());
		permitRenewalsDTO.setPermitNo(detailsForSearchedPermitNo.getPermitNo());
		permitRenewalsDTO.setRegNoOfBus(detailsForSearchedPermitNo.getRegNoOfBus());
	...
	public void onApplicationNoChange() {
		detailsForSearchedApplicationNo = permitRenewalsService
				.getAllDetailsForSearchedApplicationNo(selectedApplicationNo);
	...
	public void onQueueNoChange() {
		ApplicationNoForSearchedQueueNo = permitRenewalsService
				.getApplicationNoForSelectetedQueueNo(permitRenewalsDTO.getQueueNo());
		...
		if (currentAppNoForCallingQueueNo != null) {
			detialsForSearchedQueueNo = permitRenewalsService
					.getRecordDetailsForCurrentQueueNo(permitRenewalsDTO.getQueueNo(), currentAppNoForCallingQueueNo);
			...
			errorMsg = "You cannot search values for this Queue No.";
	...
	public void manufactureYearValidator() {
		...
		if (ominiBusDTO.getRegistrationDate() != null && ominiBusDTO.getManufactureDate() != null) {
			...
			if (manuYear > regYear) {
				errorMsg = "Year of Registration should be greater than or same as Manufacture Year";
				RequestContext.getCurrentInstance().update("frmrequiredField");
				RequestContext.getCurrentInstance().execute("PF('requiredField').show()");
	...
	public void updateRecord() throws ParseException {
		...
		if (permitRenewalsDTO.getMobileNo().isEmpty() || permitRenewalsDTO.getMobileNo().equals("")
			...
			RequestContext.getCurrentInstance().update("frmrequiredField");
			RequestContext.getCurrentInstance().execute("PF('requiredField').show()");
		} else if ((!selectedMaterialStatus.equals("") || !selectedMaterialStatus.isEmpty()
				|| selectedMaterialStatus != null)
				&& (!permitRenewalsDTO.getMobileNo().isEmpty() || !permitRenewalsDTO.getMobileNo().equals("")
						|| permitRenewalsDTO.getMobileNo() != null)
			...
			if (numCheckTel == false && numCheckMobile == false) {
				...
				searchedCurrentSeqNoForApplicationTable = permitRenewalsService.getCurrentSeqNoForPermitNo(
						selectedPermitNo, permitRenewalsDTO.getQueueNo(), selectedApplicationNo);
				searchedSeqNoForOwnerTable = permitRenewalsService.getCurrentOwnerSeqNo(selectedPermitNo,
						permitRenewalsDTO.getQueueNo(), selectedApplicationNo);
				...
				vehicleOwnerHistoryDTO = historyService.getVehicleOwnerTableData(permitRenewalsDTO.getApplicationNo(),
						sessionBackingBean.getLoginUser());
				applicationHistoryTranDTO = historyService.getApplicationTableData(permitRenewalsDTO.getApplicationNo(),
						sessionBackingBean.getLoginUser());
				...
				if (resultOwnerTable == 0) {
					...
					vehicleDTO.setTranstractionTypeCode("04");
					String currentAppNo = permitRenewalsDTO.getApplicationNo();
					...
					historyService.insertApplicationHistoryData(applicationHistoryTranDTO);

					taskDetWithAppDetDTO = new VehicleInspectionDTO();
					taskDetWithAppDetDTO = inspectionActionPointService.getDetailsWithModiFyDate(currentAppNo);
					boolean checkValueWithSameTaskCode = inspectionActionPointService
							.isCheckedSameTaskCodeForSelectedApp(currentAppNo, "PR200", "O");
					if (checkValueWithSameTaskCode == true) {
						int resultInTaskDetWithAppDet = inspectionActionPointService.updateNewTaskDetWithAppDet(
								taskDetWithAppDetDTO, sessionBackingBean.getLoginUser(), currentAppNo, "PR200", "O");
					} else {
	...
	public void searchAction() throws ParseException {
		...
		if (selectedPermitNo == null && permitRenewalsDTO.getQueueNo().equals("") && selectedApplicationNo == null) {
			...
			if (isIncompleteInspection) {
				...
				if (permitRenewalsDTO.getRegNoOfBus() != null && !permitRenewalsDTO.getRegNoOfBus().equals("")
						&& permitRenewalsDTO.getRegNoOfBus() != null) {
					...
					if ((commonDTO.getComplainNo() != null) && (commonDTO.getGrantComplainNo() == null)) {
						...
						// only have complain record
						if (commonDTO.getComplainNo() != null) {
							validationMsg = "Active/Ongoing Complain is Available.";
						}
	...
	public void searchChecking() throws ParseException {
		...
		boolean checkTaskCodePR200IsCompletedValid = permitRenewalsService
				.checkTaskCodeForCurrentAppNo(selectedApplicationNo, permitRenewalsDTO.getRegNoOfBus(), "PR200", "C");
		boolean checkTaskPM101 = false;
		if (checkTaskCodePR200IsValid == false && checkTaskCodePR200IsCompletedValid == false) {

			permitRenewalsService.insertTaskDetails(selectedApplicationNo, permitRenewalsDTO.getRegNoOfBus(), loginUser,
					"PR200", "O");
			checkTaskPM101 = permitRenewalsService.checkTaskDetails(selectedApplicationNo,
					permitRenewalsDTO.getRegNoOfBus(), "PM101", "C");
			if (checkTaskPM101 == true) {
				permitRenewalsService.CopyTaskDetailsANDinsertTaskHistory(selectedApplicationNo,
						permitRenewalsDTO.getRegNoOfBus(), loginUser, "PM101");
				permitRenewalsService.deleteTaskDetails(selectedApplicationNo, permitRenewalsDTO.getRegNoOfBus(),
						"PM101");
			}
		} else if (checkTaskCodePR200IsValid == true || checkTaskCodePR200IsCompletedValid == true) {
		...
		getPreAppNoForLatestInActiveStatusList = permitRenewalsService.getLatestInactiveAppNo(selectedPermitNo,
				permitRenewalsDTO.getRegNoOfBus());

		String previousLatestInactiveAppNo = getPreAppNoForLatestInActiveStatusList
				.get(getPreAppNoForLatestInActiveStatusList.size() - 1).getLatestPreAppNo();

		String firstThreeLettersInAppNo = previousLatestInactiveAppNo.substring(0, 3);
		if (firstThreeLettersInAppNo.equals("BAP")) {
		...
		} else if (firstThreeLettersInAppNo.equals("PAP")) {
			searchedDTO = permitRenewalsService.getSearchedDetailsWithPermitNoOrQueueNo(selectedPermitNo,
					permitRenewalsDTO.getQueueNo(), previousLatestInactiveAppNo, "I");
		} else if (firstThreeLettersInAppNo.equals("AAP")) {
			searchedDTO = permitRenewalsService.getSearchedDetailsWithPermitNoOrQueueNo(selectedPermitNo,
					permitRenewalsDTO.getQueueNo(), previousLatestInactiveAppNo, "I");
		}
		...
		permitRenewalsDTO.setQueueNo(permitRenewalsDTO.getQueueNo());
		...
		divisionSectionList = permitRenewalsService.getAllDivisionSections(selectedDistricCode);
		selectedDivisionSecCode = permitRenewalsDTO.getDivisionalSecretariatDivision();
		permitRenewalsDTO.setBeforePermitExpiredDate(searchedDTO.getBeforePermitExpiredDate());
	...
	public void updateDocuments() {
		...
		if (isOkToContinue == false) {
			...
			boolean checkValueWithSameTaskCode = inspectionActionPointService
					.isCheckedSameTaskCodeForSelectedApp(permitRenewalsDTO.getApplicationNo(), "PR200", "O");
			if (checkValueWithSameTaskCode == true) {
				int resultInTaskDetWithAppDet = inspectionActionPointService.updateNewTaskDetWithAppDet(
	...
	public void loadSearchChecking() {
		...
		setDisableCallNextBtn(true);
		setDisableSkipBtn(true);
		String currentStatusForSelectedRecord = permitRenewalsService.getCurrentStatusForSelectedRd(
				sessionBackingBean.getApplicationNo(), sessionBackingBean.getViewedPermitNo(),
				sessionBackingBean.getSelectedQueueNo(), permitRenewalsDTO.getRegNoOfBus());
		String previouesSpecialRemark = permitRenewalsService.getPreSpecialRemark(selectedPermitNo,
				permitRenewalsDTO.getRegNoOfBus());

		if (currentStatusForSelectedRecord != null) {
			if (currentStatusForSelectedRecord.equals("I")) {

			} else if (currentStatusForSelectedRecord.equals("O")) {
				searchedDTO = permitRenewalsService.getSearchedDetailsWithPermitNoOrQueueNo(
						...
						sessionBackingBean.getApplicationNo(), currentStatusForSelectedRecord);
			} else if (currentStatusForSelectedRecord.equals("A")) {
				searchedDTO = permitRenewalsService.getSearchedDetailsWithPermitNoOrQueueNo(
						sessionBackingBean.getViewedPermitNo(), sessionBackingBean.getSelectedQueueNo(),
						sessionBackingBean.getApplicationNo(), currentStatusForSelectedRecord);
			}
		} else if (currentStatusForSelectedRecord == null) {
	...
	public void checkQueueNoisValid() throws ParseException {
		...
		valueOne = migratedService.findCancelledQueueNumberForRenewals(permitRenewalsDTO.getQueueNo(), "PM101",
				"PR200");

		if (valueOne == true) {

			updateQueueNoTaskCode(permitRenewalsDTO.getQueueNo(), permitRenewalsDTO.getApplicationNo());
			searchChecking();
			updateCounterID();
		} else {
	...
	private void updateQueueNoTaskCode(String queueNo, String appNum) {

		migratedService.updateQueueNumberTaskInQueueMaster(queueNo, appNum, "PR200", "O");

	}

	private void updateCounterID() {
		if (permitRenewalsDTO.getQueueNo() != null && !permitRenewalsDTO.getQueueNo().isEmpty()
				&& !permitRenewalsDTO.getQueueNo().trim().equalsIgnoreCase("")) {

			commonService.updateCounterQueueNo(permitRenewalsDTO.getQueueNo(), sessionBackingBean.getCounterId());
														...
														if (ominiBusDTO.getInsuCat() != null
															...
															if (ominiBusDTO.getDateOfFirstReg() != null) {
																...
																if (ominiBusDTO.getSeq() != 0) {
																	...
																	ominiBusDTO.setApplicationNo(issuePermitDTO.getApplicationNo());
																	ominiBusDTO.setVehicleRegNo(issuePermitDTO.getBusRegNo());

																	/* get OminiBus Data */
																	OminiBusHistoryDTO = historyService.getOminiBusTableData(ominiBusDTO.getApplicationNo(),
																					sessionBackingBean.getLoginUser());
																	...
																	if (finalResult == 0) {
																		...
																		taskDetWithAppDetDTO = new VehicleInspectionDTO();
																		taskDetWithAppDetDTO = inspectionActionPointService.getDetailsWithModiFyDate(permitRenewalsDTO
																								.getApplicationNo());
																		boolean checkValueWithSameTaskCode = inspectionActionPointService
																				.isCheckedSameTaskCodeForSelectedApp(
	...
	public void loadValuesForViewInvestigation() {
		setInvestigationDetViewList(manageInquiryService.getInvestigationDetails(selectedPermitNo));
	...
	public void loadValuesForViewPublicComplaint() {
		complainDetViewList = manageInquiryService
				.getComplaintDetailsForPublicComplain(permitRenewalsDTO.getPermitNo());
	...
	public List<PermitRenewalsDTO> getFilterApplalicationNoList() {
		return filterApplalicationNoList;
	}

	public void setFilterApplalicationNoList(List<PermitRenewalsDTO> filterApplalicationNoList) {
		this.filterApplalicationNoList = filterApplalicationNoList;
	}
	...
	public List<PermitRenewalsDTO> getGetPreAppNoForLatestInActiveStatusList() {
		return getPreAppNoForLatestInActiveStatusList;
	}

	public void setGetPreAppNoForLatestInActiveStatusList(
			List<PermitRenewalsDTO> getPreAppNoForLatestInActiveStatusList) {
		this.getPreAppNoForLatestInActiveStatusList = getPreAppNoForLatestInActiveStatusList;
	}

	public PermitRenewalsDTO getExpiredDateDTO() {
		return ExpiredDateDTO;
	}

	public void setExpiredDateDTO(PermitRenewalsDTO expiredDateDTO) {
		ExpiredDateDTO = expiredDateDTO;
	}
	...
	public PermitRenewalsDTO getApplicationHistoryTranDTO() {
		return applicationHistoryTranDTO;
	}

	public void setApplicationHistoryTranDTO(PermitRenewalsDTO applicationHistoryTranDTO) {
		this.applicationHistoryTranDTO = applicationHistoryTranDTO;
	}
