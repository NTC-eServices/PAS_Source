...
@ManagedBean(name = "queueManagementBackingBean")
@ViewScoped
public class QueueManagementBackingBean implements Serializable {
	...
	public void onTransactionTypeChange() {
		...
		if (queueNumberDTO.getTransTypeCode() != null && !queueNumberDTO.getTransTypeCode().isEmpty()
			...
			if (queueNumberDTO.getTransTypeCode().trim().equalsIgnoreCase(PERMIT)
				...
				buttonStyleRemove();
				inspection = "false";
				vehicleEnable = true;// false;//changed on 22/10/2019 because in
										// kiosk this is taking from vehicle num
										// not app num
				permitEnable = false;
				rendrRefNoEnabe = false;// only enable when LOV value=TENDER
				appNoEnabe = false;// true;//changed on 22/10/2019 because in
									// kiosk this is taking from vehicle num not
									// app num
				appNoStarEnabe = false;
				inspectionVisible = false;
		 ...
		 * "frmQueueMananagement:centerlayout");
		 */
	}
	 ...
	 * @throws JRException
	 **/
	 public synchronized StreamedContent generateTokenListener() throws JRException {
		files = null;
		List<QueueNumberDTO> dtoList = new ArrayList<QueueNumberDTO>();
		boolean renewal = false;
		String strVehicleNo = null;

		// validate mandatory inputs - transaction type
		if (queueNumberDTO.getTransTypeCode() != null && !queueNumberDTO.getTransTypeCode().isEmpty()
				&& !queueNumberDTO.getTransTypeCode().trim().equalsIgnoreCase("")) {

			// validate inputs - application number except INSPECTION(02) all
			// the other transaction types must have an application number
			boolean validateAppNo = validateApplicationNumber(queueNumberDTO);
			strVehicleNo = queueNumberDTO.getVehicleNo();
			if (validateAppNo) {
				// validate inputs-vehicle number, if transaction type is new
				// permit(03) & inspection(02) must have a vehicle no
				boolean validateVehNo = validateVehicleNumber(queueNumberDTO);
				if (validateVehNo) {
					// validate inputs-tender ref number, if transaction type is
					// a tender(01) must have a tender ref no
					/**
					 * boolean validateTenderRefNo =
					 * validateTenderReferenceNumber(queueNumberDTO);
					...
					if (validateTenderRefNo) {
						...
						if (queueNumberDTO.getQueueService() != null && !queueNumberDTO.getQueueService().isEmpty()
								&& !queueNumberDTO.getQueueService().trim().equalsIgnoreCase("")) {

							/**
							 * If transaction type is inquiry no need to check
							 * anything just give a token number start
							 **/
							if (queueNumberDTO.getTransTypeCode().equalsIgnoreCase(INQUIRY)) {
								boolean valid = validateVehicleNoAndAppNoForInquiry(queueNumberDTO.getVehicleNo(),
										queueNumberDTO.getApplicationNo());
								if (!valid) {
									sessionBackingBean.showMessage("Warning",
											"Please enter valid Vehicle No. or Application No. ", "WARNING_DIALOG");
									return null;
								}
							...
							/**
							 * If transaction type is inquiry no need to check
							 * anything just give a token number end
							 **/

							/**
							 * If transaction type is inspection check if
							 * vehicle no is in the system start
							 **/
							if (queueNumberDTO.getTransTypeCode().equalsIgnoreCase(INSPECTION_NORMAL)) {

								/***/
								// should not allow to generate multiple IN
								// token for same vehicle no in current day
								// added by Dinushi Ranasinghe on 04-03-2020
								boolean isExistSameDay = queueManagementService.validateTokenforSameDay(strVehicleNo);
								if (isExistSameDay) {
									sessionBackingBean.showMessage("Warning",
											"Renewal process is ongoing / completed for Today.", "WARNING_DIALOG");
									return null;
								}
								/***/

								// 27/10/2019 check AM 100 C or AM106 C or AM102
								// C or AM101 C or AM103 C -> if so dont let
								// take queue numbers
								...
								if (taskCompleteInsAm) {
									sessionBackingBean.showMessage("Warning", "Inspection token cannot be generated",
											"WARNING_DIALOG");
									return null;
								}
								// 27/10/2019 check AM 100 C or AM106 C or AM102
								...
								if (exist) {
									sessionBackingBean.showMessage("Warning",
											"Inspection comepleted. Please take a renewal token", "WARNING_DIALOG");
									return null;
								}
								// 15/10/2019 check PM101 C task
								...
								if (mssage != null && !mssage.isEmpty() && !mssage.trim().equalsIgnoreCase("")) {
									sessionBackingBean.showMessage("Warning", mssage, "WARNING_DIALOG");
									return null;
								}
								// 30/5/2019 check PR200 O task

								// 15/10/2019 check inspection token is taken
								// today
								...
								if (inspectionOngoing) {
									sessionBackingBean.showMessage("Warning", "Token has already generated for today.",
											"WARNING_DIALOG");
									return null;
								}
								// 15/10/2019 check inspection token is taken
								...
								QueueNumberDTO dto = queueManagementService.validateUserInputValues(queueNumberDTO);// checks
																													// data
																													// is
																													// available
																													// for
																													...
																													// in
																													// nt_t_pm_application
								boolean isExist = queueManagementService
										.validateDuplicateQueueNoGenerateInQueuMaster(queueNumberDTO);
								if (dto != null || isExist) {
							...
							/**
							 * If transaction type is inspection check if
							 * vehicle no is in the system end
							 **/

							/**
							 * If transaction type inspection amendment start
							 **/
							if (queueNumberDTO.getTransTypeCode().equalsIgnoreCase(INSPECTION_AMEND)) {
								...
								if (dto != null && dto.getApplicationNo() != null && !dto.getApplicationNo().isEmpty()
									...
									if (!taskValid) {
										sessionBackingBean.showMessage("Warning", "Application number is not eligible",
												"WARNING_DIALOG");
										return null;
									}
								}
								...
								QueueNumberDTO queueDto = queueManagementService
										.validateUserInputValueForInspectionAmmendment(queueNumberDTO);// checks
																										// data
																										// is
																										// available
																										// for
								...
								if (queueDto != null) {
									...
									boolean generateAgain = queueManagementService
											.validateDuplicateQueueNoGenerate(queueDto);
									if (!generateAgain) {
										sessionBackingBean.showMessage("Warning", "Token has already generated",
												"WARNING_DIALOG");
										return null;
									...
									// **Validate same details generated queue
									// token for same queue type again end**//*
							...
							/** If transaction type inspection amendment end **/

							/**
							 * If transation type is New Permit check if it's
							 * pm_is_back_log not equal to 'Y' start
							 **/
							if (queueNumberDTO.getTransTypeCode().equalsIgnoreCase(PERMIT)) {
								// TODO ask whether select needs to be checked
								// with pm_created_date today?
								boolean isBackLog = queueManagementService
										.validateApplicationNoIsBackLog(queueNumberDTO.getApplicationNo());

								if (isBackLog) {
									// if it is backlog application should not
									// let proceed with application number
									sessionBackingBean.showMessage("Warning",
							...
							/**
							 * If transaction type is New Permit check if it's
							 * pm_is_back_log not equal to 'Y' end
							 **/

							/**
							 * if transaction type is RENEWAL should check
							 * is_new_permit='N' or is_back_log='Y' start
							 **/
							...
							/**
							 * if transaction type is RENEWAL should check
							 * is_new_permit='N' or is_back_log='Y' end
							 **/

							/**
							 * check amendments validation whether to check if
							 * the entered data is active start
							 **/
							if (queueNumberDTO.getTransTypeCode().equalsIgnoreCase(AMEND_BUS)
								...
								// 25/10/2019 start
								/**
								 * check new validation -> if PM400 C let the
								 * user take the token or else give the
								 * application number to continue the process
								 **/
								String appNum = queueManagementService.checkProcessIsDoneForAmmendment(queueNumberDTO);
								if (appNum != null && !appNum.isEmpty() && !appNum.trim().equalsIgnoreCase("")) {
									sessionBackingBean.showMessage("Warning",
											"Please complete ammendment process. Your application no is " + appNum,
											"WARNING_DIALOG");
									return null;
								}
								...
								if (!valid) {
									sessionBackingBean.showMessage("Warning", "Entered details are not Active stage",
											"WARNING_DIALOG");
									return null;
								}
								...
								if (ongoing) {
									sessionBackingBean.showMessage("Warning",
											"Queue Number has already genetated for entered details", "WARNING_DIALOG");
									return null;
								}
							}
							/**
							 * check amendments validation whether to check if
							 * the entered data is active end
							 **/

							/** Check entered inputs are valid start **/
							String returnVal = validateBeforeGenerate(queueNumberDTO, renewal);

							if (returnVal != null && !returnVal.isEmpty() && !returnVal.trim().equalsIgnoreCase("")) {
							...
							// if it's amendments no need to check transaction
							// type
							if (!queueNumberDTO.getTransTypeCode().equalsIgnoreCase(AMEND_BUS)
									&& !queueNumberDTO.getTransTypeCode().equalsIgnoreCase(AMEND_SERVICE)
									&& !queueNumberDTO.getTransTypeCode().equalsIgnoreCase(AMEND_OWNER)
								...
								if (!queueNoValidForTransType) {
									if (queueNumberDTO.getTransTypeCode().trim().equalsIgnoreCase(CANCEL)) {
										// cancel duplicate will be checked here
										sessionBackingBean.showMessage("Warning", "Token has already generated",
												"WARNING_DIALOG");
									} else {
										// except cancel
										// queueNoValidForTransType checks
										// whether transaction type is valid or
										// not
										sessionBackingBean.showMessage("Warning",
							...
							/**
							 * Validate same details generated queue token for
							 * same queue type again start
							 **/
							boolean generateAgain = queueManagementService
									.validateDuplicateQueueNoGenerate(queueNumberDTO);
							if (!generateAgain) {
								sessionBackingBean.showMessage("Warning", "Token has already generated",
										"WARNING_DIALOG");
								return null;
							...
							/**
							 * if trans type permit holder change / permit
							 * holder & bus change get former app num and insert
							 * to queue_mastart start
							 **/

							/**
							 * if trans type permit holder change / permit
							 * holder & bus change get former app num and insert
							 * to queue_mastart end
							 **/

							String queueNo = queueManagementService.insertQueueDataIntoNT_M_QUEUE_MASTER(queueNumberDTO,
							...
							// To avoid displaying NULL for owner name in queue
							// ticket. In case it's null
							if (ownerName != null && !ownerName.isEmpty() && !ownerName.trim().equalsIgnoreCase("")) {
							} else {
								ownerName = " ";
							}

							// TODO this is just to test. This Report will be
							// removed once token printer comes
							/** TO BE REMOVED START **/
							...
							sessionBackingBean.showMessage("Warning", "Please select Queue Service", "WARNING_DIALOG");
						}
					} else {
	...
	public boolean validateVehicleNumber(QueueNumberDTO queueNumberDTO) {
		...
		// if INSPECTION_NORMAL(02) must have a vehicle no
	...
	/** generate token if it is a inquiry start **/
	 public synchronized String tokenGenerateForInquiry(QueueNumberDTO queueNumberDTO) {
