 ...
 */
@ViewScoped
@ManagedBean(name = "simEditBean")

public class SimRegistrationViewEditBackingBean {
	...
	public SimRegistrationViewEditBackingBean() {
		...
		simDetailsList = simRegEditService.getSIMDetails();
	...
	public void searchAction() {
		...
		if ((simSearchDTO.getSimRegNo().isEmpty() || simSearchDTO.getSimRegNo().equals(null))
			...
			if (list == null) {
				...
				buttonDisabled(simRegistrationDTO.getSimRegNo());
	...
	public void buttonDisabled(String regNo) {
		int val = simRegistrationService.checkRegNoStatus(regNo);
		if (val == 0) {
	...
	public void saveBtnAction() {
		...
		if (validatSave()) {
			...
			simRegistrationService.addSimRegistration(simRegistrationDTO);
			...
			for (SimRegistrationDTO dto : emiDTOList) {
				...
				simRegistrationService.addEmi(dto);
			}
	...
	public void updateBtnAction() {
		...
		if (validatSave()) {
			...
			simRegistrationService.updateSimRegistration(simRegistrationDTO);
			...
			for (SimRegistrationDTO dto : emiDTOList) {
				...
				simRegistrationService.updateEmi(dto);
			}
	...
	public void cancelVoucherBtnAction() {
		...
		if (simRegistrationDTO.getVoucherApprovedStatus().equals("A")) {
			...
			simRegistrationService.updateApproveRejectVoucher(voucher.getVouNo(), "VC", null,
					sessionBackingBean.getLoginUser());
			/** end **/

			/* update sim_status_type as R -> SIM Registration */
			simRegEditService.updateSimRegStatus(simRegistrationDTO.getSimRegNo(), "R",
					sessionBackingBean.getLoginUser());
			/* end */
