...
public static String getDecimalValue(String decimal){
    ...
    if(decimal.contains(".")){
        ...
        if(partAfterDecimalPoint.length()>0)
            output+= ((output.length()>0?" rupees  and ":"")+getWholeWord(Integer.parseInt(partAfterDecimalPoint))+"cents only.");

    }else{
        output=getWholeWord(Integer.parseInt(decimal))+"RUPESS ONLY.";
    }
    return output; 
}
